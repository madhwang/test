...
public class TimeRecordDAO {
	...
	public int insertOfPunchIn(final String memberNo) {
		...
		return this.sqlSession.insert(TimeRecordDAO.NAME_SPACE + "insertOfPunchIn", timeRecord);
	}

	public TimeRecord selectOneOfMemberToday(final String memberNo) {
		TimeRecord timeRecord = new TimeRecord();
		timeRecord.setMemberNo(memberNo);
		return this.sqlSession.selectOne(TimeRecordDAO.NAME_SPACE + "selectOneOfMemberToday", timeRecord);
	}
	...
	public List<TimeRecord> selectRecordsOfMember(final int year, final int month, final String memberNo) {
		...
		return this.sqlSession.selectList(TimeRecordDAO.NAME_SPACE + "selectRecordsOfMember", param);
	}

	public int updateOfPunchOut(final String memberNo) {
		TimeRecord timeRecord = new TimeRecord();
		timeRecord.setMemberNo(memberNo);
		return this.sqlSession.update(TimeRecordDAO.NAME_SPACE + "updateOfPunchOut", timeRecord);
	}

	public List<TimeStat> selectWorkingSecondsOfMonth(final Integer year, final Integer month) {
		HashMap<String, String> param = new HashMap<String, String>();
		param.put("year", year.toString());
		param.put("month", month.toString());
		param.put("yearMonth", String.format("%d%02d", year, month));
		param.put("nextYearMonth", String.format("%d%02d", year, month + 1));
		return this.sqlSession.selectList(TimeRecordDAO.NAME_SPACE + "selectWorkingSecondsOfMonth", param);
	}
}